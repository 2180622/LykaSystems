<div class="container-fluid " style="font-size:14px; height:70%">
    ...
    @if($Notificacoes)
        ...
        @foreach($Notificacoes as $Notificacao)
            ...
            @endif
            @if($Notificacao->data['dataComeco'] <= (new DateTime()))
...
<script>
    function show(div){
        div.find('.descricao').first().css({"display":"block"});
        div.find('.mostra').first().css({"display":"none"});
    }
    function hide(div){
        div.find('.descricao').first().css({"display":"none"});
        div.find('.mostra').first().css({"display":"block"});
    }
</script>
